...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const frequency = searchParams.get('frequency') as 'daily' | 'weekly' | 'monthly' | 'once' | 'trigger' | null
        const search = searchParams.get('search') || undefined

        logRequest('GET', url, { page, limit, status, frequency, search })
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const result = await ScheduledNotificationService.createScheduledNotification(data)
        ...
        console.error('Scheduled Notifications API error:', error)
        return NextResponse.json(
            { success: false, error: 'Failed to create scheduled notification' },
            { status: 500 }
        )
    }
}