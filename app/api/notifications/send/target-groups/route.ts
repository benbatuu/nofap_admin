...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const result = await NotificationSendService.getTargetGroups()
        ...
        console.error('Target Groups API error:', error)
        return NextResponse.json(
            { success: false, error: 'Failed to fetch target groups' },
            { status: 500 }
        )
    }
} 