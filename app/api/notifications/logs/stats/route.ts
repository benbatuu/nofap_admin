...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const result = await NotificationLogService.getNotificationLogStats()
        ...
        console.error('Notification Log Stats API error:', error)
        return NextResponse.json(
            { success: false, error: 'Failed to fetch notification log stats' },
            { status: 500 }
        )
    }
} 